...
def patch_profile(
    ...
    UNAUTHORIZED_ROLES = (
        models.UserRole.CUSTOMER.value,
        models.UserRole.EMPLOYEE.value,
    )
    if current_user.role in UNAUTHORIZED_ROLES:
        raise HTTPException(
            status_code=status.HTTP_401_UNAUTHORIZED,
            detail=f"The '{current_user.role}' role is not allowed to update user role.")
